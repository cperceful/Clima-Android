...
public class WeatherController extends AppCompatActivity {
    ...
    //handles user granting or denying permission. Like onClick(), this listens for a request response
    @Override
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        super.onRequestPermissionsResult(requestCode, permissions, grantResults);
        //check if requestCode in callback matches constant
        if (requestCode == REQUEST_CODE){
            //check if grantResults has at least one entry and the first entry is permission granted
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                Log.d("Clima", "onRequestPermissionsResult(): granted");
                //permission has been granted, retry call to getWeatherForCurrentLocation();
                getWeatherForCurrentLocation();
            } else {
                Log.d("Clima", "Permission denied, so sad :(");
            }
        }
    }
